...
import 'package:fori_feed/firebase_service.dart';
...
class _CategSportsState extends State<CategSports> {
  ...
  Future<void> _fetchGoogleNews() async {
    await _fetchNews(
      'Google News',
      'https://news.google.com/rss/topics/CAAqKggKIiRDQkFTRlFvSUwyMHZNRFp1ZEdvU0JXVnVMVWRDR2dKUVN5Z0FQAQ?hl=en-PK&gl=PK&ceid=PK%3Aen&oc=11',
      (news) => setState(() {
  ...
  Future<void> _fetchExpressTribuneNews() async {
    try {
      ...
      await _fetchNews(
        'Express Tribune',
        'https://tribune.com.pk/feed/cricket',
        (news) {
  ...
  Future<void> _fetchBolNews() async {
    try {
      ...
      await _fetchNews(
        "Bol News",
        'https://www.bolnews.com/sports/cricket/feed/',
        (news) {
          combinedNews.addAll(news);
        },
      );
      await _fetchNews(
        "Bol News",
        'https://www.bolnews.com/sports/feed/?paged=2',
        (news) {
          combinedNews.addAll(news);
        },
      );
      await _fetchNews(
        "Bol News",
        'https://www.bolnews.com/sports/feed/?paged=3',
        (news) {
          combinedNews.addAll(news);
        },
      );
      await _fetchNews(
        "Bol News",
        'https://www.bolnews.com/sports/feed/?paged=4',
        (news) {
          combinedNews.addAll(news);
        },
      );
      await _fetchNews(
        "Bol News",
        'https://www.bolnews.com/sports/feed/?paged=5',
        (news) {
          combinedNews.addAll(news);
        },
      );
  ...
  Future<void> _fetchHumNews() async {
    try {
      ...
      await _fetchNews(
        "Hum News",
        'https://humnews.pk/sports/feed/?paged=2',
        (news) {
          combinedNews.addAll(news);
        },
      );
      await _fetchNews(
        "Hum News",
        'https://humnews.pk/sports/feed/?paged=3',
        (news) {
          combinedNews.addAll(news);
        },
      );
      await _fetchNews(
        "Hum News",
        'https://humnews.pk/sports/feed/?paged=4',
        (news) {
          combinedNews.addAll(news);
        },
      );
      await _fetchNews(
        "Hum News",
        'https://humnews.pk/sports/feed/?paged=5',
        (news) {
          combinedNews.addAll(news);
        },
      );
  ...
  void _submitComment() {
    ...
    if (commentText.isNotEmpty) {
      // You can store the comment or display it
                        ...
                        children: [
                          ...
                          ElevatedButton(
                            onPressed: () async {
                              ...
                              //       index]; // Trigger rebuild to display the comment
                              // });
